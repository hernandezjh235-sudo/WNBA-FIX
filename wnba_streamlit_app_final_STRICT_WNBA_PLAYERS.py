...
# ============================================================
# WNBA PLAYER VALIDATION
# ============================================================
# Underdog sometimes labels all basketball props generically, which can leak NBA
# players into the WNBA app. This validation keeps the prop engine alive while
# rejecting obvious NBA rows.

WNBA_PLAYER_ALLOWLIST = {
    # Las Vegas Aces
    "aja wilson", "chelsea gray", "jackie young", "kelsey plum", "alysha clark",
    "kiah stokes", "meg gustafson", "kate martin", "sydney colson",
    # New York Liberty
    "breanna stewart", "sabrina ionescu", "jonquel jones", "courtney vandersloot",
    "betnijah laney", "nyara sabally", "leonie fiebich", "kennedy burke",
    # Indiana Fever
    "caitlin clark", "aliyah boston", "kelsey mitchell", "lexie hull",
    "temi fagbenle", "naLyssa smith".lower(), "erica wheeler",
    # Minnesota Lynx
    "napheesa collier", "kayla mcbride", "courtney williams", "alanna smith",
    "bridget carleton", "dorka juhasz", "diamond miller",
    # Connecticut Sun
    "alyssa thomas", "dewanna bonner", "brionna jones", "di jonai carrington",
    "dijonai carrington", "tyasha harris", "marina mabrey",
    # Phoenix Mercury
    "kahleah copper", "brittney griner", "diana taurasi", "natasha cloud",
    "sophie cunningham", "bec allen", "michaela onyenwere",
    # Seattle Storm
    "jewell loyd", "nneka ogwumike", "skylar diggins", "ezi magbegor",
    "gabby williams", "sami whitcomb", "jordan horston",
    # Dallas Wings
    "arike ogunbowale", "satou sabally", "teaira mccowan", "natasha howard",
    "jacy sheldon", "maddy siegrist", "kalani brown",
    # Chicago Sky
    "angel reese", "kamilla cardoso", "chennedy carter", "marina mabrey",
    "elizabeth williams", "dana evans", "diamond deshields",
    # Atlanta Dream
    "rhyne howard", "allisha gray", "tina charles", "cheyenne parker",
    "naz hillmon", "jordin canada", "aerial powers",
    # Los Angeles Sparks
    "dearica hamby", "rickea jackson", "cameron brink", "azura stevens",
    "kia nurse", "lexie brown", "rae burrell",
    # Washington Mystics
    "aaliyah edwards", "brittney sykes", "shakira austin", "ariel atkins",
    "stefanie dolson", "julie vanloo", "emily engstler",
    # Golden State Valkyries / newer common WNBA names
    "kate martin", "kayla thornton", "tiffany hayes", "monique billings",
    "cecilia zandalasini", "temi fagbenle",
}
...
NBA_PLAYER_BLOCKLIST = {
    ...
    "deaaron fox", "domantas sabonis", "lauri markkanen",
}
...
NBA_TEAM_CONTEXT_REJECT = {
    ...
    "san antonio spurs", "pistons", "spurs", "warriors", "lakers", "clippers",
    "mavericks", "rockets", "grizzlies", "pelicans", "blazers", "thunder",
    "nuggets", "jazz", "kings", "celtics", "nets", "knicks", "sixers",
    "raptors", "bulls", "cavaliers", "pacers", "bucks", "hawks", "hornets",
    "heat", "magic"
}

def is_valid_wnba_prop_player(player, *objs):
    """Final player-level guard for WNBA props.

    Accepts known WNBA players and rejects obvious NBA players/team contexts.
    This prevents generic Underdog basketball props from leaking NBA rows into
    the WNBA prop board.
    """
    ...
    if any(term in clean_blob for term in NBA_TEAM_CONTEXT_REJECT):
        # If the player is explicitly on the WNBA allowlist, keep them.
        return p in WNBA_PLAYER_ALLOWLIST

    if p in WNBA_PLAYER_ALLOWLIST:
        return True

    # If the raw object clearly says WNBA/women and does not contain NBA markers,
    # allow it so new/less common WNBA players can still show.
    if any(x in clean_blob for x in ["wnba", "women", "womens", "women s basketball"]):
        if " nba " not in f" {clean_blob.replace('wnba', 'w n b a')} ":
            return True
...
def build_prop_board(games, selected_props, bankroll, default_odds, max_players_per_game=26):
    ...
    diagnostics = [{
        ...
        "Details": "Direct Underdog WNBA parser active with strict WNBA player validation. NBA rows are rejected.",
    }]
