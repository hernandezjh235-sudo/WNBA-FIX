...
# ============================================================
# WNBA PLAYER SAFETY FILTER
# ============================================================
# Underdog sometimes returns generic basketball props. This filter blocks obvious
# NBA players/teams while still allowing WNBA rows that Underdog labels only as
# "basketball" instead of "WNBA".
...
def is_valid_wnba_prop_player(player, *objs):
    """Balanced WNBA prop filter.

    Rejects obvious NBA rows, but does not require a complete WNBA roster allowlist.
    This prevents NBA leaks while still letting real WNBA props appear when Underdog
    only marks them as generic basketball.
    """
    ...
    # Explicit WNBA/team context always passes unless player is blocklisted.
    if any(term in clean_blob for term in WNBA_TEAM_CONTEXT_ACCEPT):
        return True

    # Obvious NBA team context gets rejected.
    if any(term in clean_blob for term in NBA_TEAM_CONTEXT_REJECT):
        return False

    # Explicit NBA league context gets rejected, while WNBA is allowed.
    nba_check = f" {clean_blob.replace('wnba', 'w n b a')} "
    if " nba " in nba_check or "national basketball association" in nba_check:
        return False

    # If Underdog only says generic basketball, allow it unless blocked above.
    return True
...
def build_prop_board(games, selected_props, bankroll, default_odds, max_players_per_game=26):
    ...
    diagnostics = [{
        ...
        "Details": "Direct Underdog WNBA parser active with balanced NBA-player blocking. Generic basketball rows are allowed unless obvious NBA.",
    }]
